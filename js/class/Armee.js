...
class Armee {
	constructor(parametres = {}) {
		...
		* @type array
		*/
		this._unite = new Array(15).fill(0);
	...
	*
	* @method parseArmee
	* @param {String} texte - L'armée sous forme de texte.
	*/
	parseArmee(texte) {
		let RegExpToutSaufChiffre = new RegExp("[^0-9]", "g"); //Capture tout sauf les chiffres
		let r0 = new RegExp("(Ouvrières?)", "gi");
		let r1 = new RegExp("(\\bgs\\b)|(Goules? Sauvages?)|(\\bjsn\\b)|(Jeunes? Soldates? Naines?)|(Young dwarf)|(Young dwarves)", "gi");
		let r2 = new RegExp("(\\bg\\b)|(Goules?)|(\\bsn\\b)|(Soldates? Naines?)|(Dwarf)|(Dwarves)", "gi");
		let r3 = new RegExp("(\\bcd?g\\b)|(Chefs? des Goules?)|(\\bne\\b)|(Naines? d')|(Naines? d’)|(Top dwarf)|(Top dwarves)", "gi");
		let r4 = new RegExp("(\\bmer?\\b)|(Mercenaires?)|(\\bjs\\b)|(Jeunes? Soldates?)|(Young soldiers?)", "gi");
		let r5 = new RegExp("(\\bpil?\\b)|(Pillards?)|(\\bs\\b)|(Soldates?)|(Soldiers?)", "gi");
		let r6 = new RegExp("(\\pro?\\b)|(Protectrons?)|(\\bc\\b)|(Concierges?)|(doorkeepers?)", "gi");
		let r7 = new RegExp("(\\bce\\b)|(Concierges? d')|(Concierges? d’)|(Top doorkeepers?)", "gi");
		let r8 = new RegExp("(\\bmut?\\b)|(Mutants?)|(\\ba\\b)|(Artilleuses?)|(Fire ants?)", "gi");
		let r9 = new RegExp("(\\bsm\\b)|(\\bsmut?\\b)|(Supers? Mutants?)|(\\bae\\b)|(Artilleuses? d')|(Artilleuses? d’)|(Top fire ants?)", "gi");
		let r10 = new RegExp("(\\bcom?\\b)|(Commandos?)|(\\bse\\b)|(Soldates? d')|(Soldates? d’)|(Top soldiers?)", "gi");
		let r11 = new RegExp("(\\bnu?\\b)|(Nucleotrons?)|(\\bta\\b)|(\\btk\\b)|(Tanks?)", "gi");
		let r12 = new RegExp("(\\btae\\b)|(\\btke\\b)|(Tanks? d')|(Tanks? d’)|(Top tanks?)", "gi");
		let r13 = new RegExp("(\\bche?\\b)|(Chevaliers?)(\\btu\\b)|(Tueuses?)|(Killers?)", "gi");
		let r14 = new RegExp("(\\bpa?\\b)|(Paladins?)|(\\btue\\b)|(Tueuses? d'?)|(Tueuses? d’?)|(Top killers?)", "gi");
		let unite = new Array(r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13, r14);
		...
		// L'ordre est important pour éviter les correspondances partielles
		let ordre = new Array(0, 1, 3, 2, 4, 10, 5, 9, 8, 7, 6, 12, 11, 14, 13);
	...
	*/
	getTemps(tdp) {
		return this._unite.reduce((acc, val, i) => { return acc + (val * TEMPS_UNITE[i] * Math.pow(0.9, tdp)) }, 0);
	}
	/**
	...
	*/
	getBaseVie() {
		return this._unite.reduce((acc, val, i) => { return acc + (val * VIE_UNITE[i]) }, 0);
	}
	/**
	...
	*/
	getBaseAtt() {
		return this._unite.reduce((acc, val, i) => { return acc + (val * ATT_UNITE[i]) }, 0);
	}
	/**
	...
	*/
	getBaseDef() {
		return this._unite.reduce((acc, val, i) => { return acc + (val * DEF_UNITE[i]) }, 0);
	}
	/**
	...
	*/
	getConsommation(lieu) {
		return this._unite.reduce((acc, val, i) => { return acc + (val * COUT_UNITE[i] * 0.05 * lieu) }, 0);
	}
	/**
	...
	*/
	async calculChasse(tdcDep, diffChasse, fixNB, fixHF, reste) {
		...
		if (!fixHF) {
			let bBoucle = (iHuntCm2 > 500000000000);
