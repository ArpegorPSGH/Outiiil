    ...
    toHTMLMessagerie()
    {
		...
		if(this._armeeEnnemieAp.getSommeUnite()){
			...
			html += `<tr><td><img width='18' height='18' src='images/icone/horloge.png' class='o_vAlign'/></td><td class='right'>${Utils.intToTime(this._armeeEnnemieAp.getTemps(0))}</td>
				...
				<tr><td>Perte JSN 10%</td><td class='right'>${numeral(Math.round(this._armeeEnnemieAp.getTotalDef(this._defenseur.niveauRecherche[2]) / 10 / (8 + 8 * monProfilJoueur.niveauRecherche[1] / 10))).format()} <img src='images/attention.gif' alt='attention' title='Avec votre bonus Bouclier de : ${monProfilJoueur.niveauRecherche[1]}' class='o_vAlign'/></td>
		...
		if(this._armeePe.getSommeUnite()){
			...
			html += `<span style='text-decoration:underline;' class='gras'>Bilan après combat</span><br/><table class='o_tabAnalyse right' cellspacing='0'>
				...
				<tr><td>${IMG_VIE}</td><td class='right'>${(vieHB > 0 ? '+' : '') + numeral(vieHB).format()}(HB)</td><td class='right'>${(vieHB > 0 ? '+' : '') + numeral(this._armeeAp.getTotalVie(monProfilJoueur.niveauRecherche[1]) - this._armeeAv.getTotalVie(monProfilJoueur.niveauRecherche[1])).format()}(AB)</td><td style='padding-left:10px'>${(vieHB > 0 ? '+' : '') + (vieHB * 100 / this._armeeAv.getBaseVie()).toFixed(2)}%</td></tr>
				<tr><td>${IMG_ATT}</td><td style='padding-left:10px' class='right'>${(attHB > 0 ? '+' : '') + numeral(attHB).format()}(HB)</td><td style='padding-left:10px' class='right'>${(attHB > 0 ? '+' : '') + numeral(this._armeeAp.getTotalAtt(monProfilJoueur.niveauRecherche[2]) - this._armeeAv.getTotalAtt(monProfilJoueur.niveauRecherche[2])).format()}(AB)</td><td style='padding-left:10px'>${(attHB > 0 ? '+' : '') + (attHB * 100 / this._armeeAv.getBaseAtt()).toFixed(2)}%</td></tr>
				<tr><td>${IMG_DEF}</td><td class='right'>${(defHB > 0 ? '+' : '') + numeral(defHB).format()}(HB)</td><td class='right'>${(defHB > 0 ? '+' : '') + numeral(this._armeeAp.getTotalDef(monProfilJoueur.niveauRecherche[2]) - this._armeeAv.getTotalDef(monProfilJoueur.niveauRecherche[2])).format()}(AB)</td></td><td style='padding-left:10px'>${(defHB > 0 ? '+' : '') + (defHB * 100 / this._armeeAv.getBaseDef()).toFixed(2)}%</td></tr>
				</table><br/>`;
		}
		html += "</div>";
        return html;
