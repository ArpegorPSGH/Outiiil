...
* @constructor
*/
Utils.register(class Messagerie extends Page {
    ...
    */
    analyseMessage() {
        ...
        const observer = new MutationObserver((mutations) => {
            // On utilise setTimeout(..., 0) pour laisser le temps aux scripts natifs de Fourmizzz 
            // de terminer leur exécution (notamment le calcul du défilement/scroll)
            setTimeout(async () => {
                    ...
                    for (const node of mutation.addedNodes) {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            ...
                            if (element.hasClass("contenu_conversation")) {
                                ...
                                const titreMess = header.find(".td_objet").text();
                                ...
                                const container = element.closest(".contenu_conversation");
                                if (container.length) {
                                    const header = container.prevAll("[id^='conversation_']:first");
                                    if (header.length) {
                                        const titreChasse = header.find(".td_objet").text();
