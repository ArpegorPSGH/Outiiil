...
class Combat {
    ...
    */
    async toHTMLMessagerie() {
        ...
        if (this._pointDeVue == 1)
            ...
            bonusEnnemie = `${this._defenseurBonusLieu.length ? "Bouclier (/ " + LIBELLE_LIEU[this._lieu] + ") : " + this._defenseurBonusLieu.join(" - ") + " | " : (recherchesDefenseur[1] != -1 ? "Bouclier : " + recherchesDefenseur[1] + " | " : "")}Armes : ${recherchesDefenseur[2]}`;
        ...
        if (this._armeeEnnemieAp.getSommeUnite()) {
            ...
            if (recherchesDefenseur[1] != -1)
                html += `<td class='right'>${numeral(this._armeeEnnemieAp.getTotalVie(recherchesDefenseur[1])).format()} (AB)<img src='images/attention.gif' alt='attention' title='Vie sans le bonus lieu !' class='o_vAlign'/></td></tr>`;
            ...
            html += `<tr><td><img width='18' height='18' src='images/icone/horloge.png' class='o_vAlign'/></td><td class='right'>${Utils.intToTime(this._armeeEnnemieAp.getTemps(0))}</td>
                <td class='right' style='width:30px;'>${IMG_ATT}</td><td class='right'>${numeral(this._armeeEnnemieAp.getBaseAtt()).format()} (HB)</td><td class='right'>${numeral(this._armeeEnnemieAp.getTotalAtt(recherchesDefenseur[2])).format()} (AB)</td></tr>
				<tr><td>Perte JSN 10%</td><td class='right'>${numeral(Math.round(this._armeeEnnemieAp.getTotalDef(recherchesDefenseur[2]) / 10 / (8 + 8 * recherches[1] / 10))).format()} <img src='images/attention.gif' alt='attention' title='Avec votre bonus Bouclier de : ${recherches[1]}' class='o_vAlign'/></td>
				<td class='right' style='width:30px;'>${IMG_DEF}</td><td class='right'>${numeral(this._armeeEnnemieAp.getBaseDef()).format()} (HB)</td><td class='right'>${numeral(this._armeeEnnemieAp.getTotalDef(recherchesDefenseur[2])).format()} (AB)</td></tr>
				</table><br/>`;
        }
        ...
        if (this._armeePe.getSommeUnite()) {
            ...
            html += `<span style='text-decoration:underline;' class='gras'>Bilan après combat</span><br/><table class='o_tabAnalyse right' cellspacing='0'>
				...
				<tr><td>${IMG_VIE}</td><td class='right'>${(vieHB > 0 ? '+' : '') + numeral(vieHB).format()}(HB)</td><td class='right'>${(vieHB > 0 ? '+' : '') + numeral(this._armeeAp.getTotalVie(recherches[1]) - this._armeeAv.getTotalVie(recherches[1])).format()}(AB)</td><td style='padding-left:10px'>${(vieHB > 0 ? '+' : '') + (vieHB * 100 / this._armeeAv.getBaseVie()).toFixed(2)}%</td></tr>
				<tr><td>${IMG_ATT}</td><td style='padding-left:10px' class='right'>${(attHB > 0 ? '+' : '') + numeral(attHB).format()}(HB)</td><td style='padding-left:10px' class='right'>${(attHB > 0 ? '+' : '') + numeral(this._armeeAp.getTotalAtt(recherches[2]) - this._armeeAv.getTotalAtt(recherches[2])).format()}(AB)</td><td style='padding-left:10px'>${(attHB > 0 ? '+' : '') + (attHB * 100 / this._armeeAv.getBaseAtt()).toFixed(2)}%</td></tr>
				<tr><td>${IMG_DEF}</td><td class='right'>${(defHB > 0 ? '+' : '') + numeral(defHB).format()}(HB)</td><td class='right'>${(defHB > 0 ? '+' : '') + numeral(this._armeeAp.getTotalDef(recherches[2]) - this._armeeAv.getTotalDef(recherches[2])).format()}(AB)</td></td><td style='padding-left:10px'>${(defHB > 0 ? '+' : '') + (defHB * 100 / this._armeeAv.getBaseDef()).toFixed(2)}%</td></tr>
				</table><br/>`;
        }
        html += "</div>";
        return html;
    ...
    async toHTMLBoite() {
        ...
        for (let i = 0; i < 15; i++) {
            if (this._armeeAv.unite[i] || this._armeeEnnemieAv.unite[i] || this._armeeAp.unite[i]) {
                ...
                html += `<tr><td>${NOM_UNITE[i]}</td><td>${numeral(this._armeeAp.unite[i]).format()} (${(diff1 > 0 ? "+" : "") + numeral(diff1).format()})</td><td>${numeral(this._armeeEnnemieAp.unite[i]).format()} (${numeral(this._armeeEnnemieAp.unite[i] - this._armeeEnnemieAv.unite[i]).format()})</td></tr>`;
            }
        }
    ...
    */
    async simuler() {
        let recherchesAttaquant = await this._attaquant.lire('Niveaux Recherches');
        let recherchesDefenseur = await this._defenseur.lire('Niveaux Recherches');
        let constructionsDefenseur = await this._defenseur.lire('Niveaux Constructions');
        ...
        while (vieAttaquant.reduce((acc, val) => { return acc + val; }, 0) > 0 && vieDefenseur.reduce((acc, val) => { return acc + val; }, 0) > 0) {
            ...
            bonusDegatAtt = baseDegatAtt * recherchesAttaquant[2] / 10;
