/*
 * FonctionnaliteForumAdminUtils.js
 * Hraesvelg
 **********************************************************************/
 ...
 *
 * @class FonctionnaliteForumAdminUtils
 * @extends {FonctionnaliteAlliance}
 */
Utils.register(class FonctionnaliteForumAdminUtils extends FonctionnaliteAlliance {

    static ABREVIATIONS_HISTORY = ['FAU'];
        ...
        if ($("img[src='images/icone/outil.gif']").length && !$("#o_afficheMenuUtilitaire").length) {
            ...
            $("#o_creerUtilitaire").click(async (e) => {
                if (nomsSectionsRequis) {
                            ...
                            try {
                                ...
                                if (idCat) {
                                    ...
                                    Utils.modifierSection(nomSection, idCat, "cache").then((data) => {
                                        ...
                                        console.error(`[FonctionnaliteForumAdminUtils][o_creerUtilitaire] Erreur lors de la modification de la section ${nomSection} (ID: ${idCat}):`, textStatus, errorThrown);
                                        $.toast({ ...TOAST_ERROR, text: `Une erreur réseau a été rencontrée lors de la protection de la section ${nomSection}.` });
                                    });
                                } else {
                                    console.warn(`[FonctionnaliteForumAdminUtils][o_creerUtilitaire] ID de section non retourné pour "${nomSection}".`);
                                }
                            } catch (error) {
                                console.error(`[FonctionnaliteForumAdminUtils][o_creerUtilitaire] Erreur lors de la création de la section ${nomSection}:`, error);
                    ...
                    console.warn("[FonctionnaliteForumAdminUtils][o_creerUtilitaire] nomsSectionsRequis n'est pas défini. Aucune section à créer.");
                }
                return false;
            });
