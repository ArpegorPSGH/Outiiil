...
class Chasse {
	...
	*/
	async toHTMLMessagerie() {
		...
		let tdp = await monProfilJoueur.getTDP(),
			...
			bonusAtt = this._armeeAp.getTotalAtt(recherches[2]) - this._armeeAv.getTotalAtt(recherches[2]),
			bonusDef = this._armeeAp.getTotalDef(recherches[2]) - this._armeeAv.getTotalDef(recherches[2]),
			bonusVie = this._armeeAp.getTotalVie(recherches[1]) - this._armeeAv.getTotalVie(recherches[1]),
	...
	*/
	async toHTMLBoite(bVisible) {
		...
		const diffDef = this._armeeAp.getTotalDef(recherches[2]) - this._armeeAv.getTotalDef(recherches[2]);

		const sommeAv = this._armeeAv.getSommeUnite() || 1;
		const vieAv = this._armeeAv.getTotalVie(recherches[1]) || 1;
		const attAv = this._armeeAv.getTotalAtt(recherches[2]) || 1;
		const defAv = this._armeeAv.getTotalDef(recherches[2]) || 1;

		let html = `<tr ${bVisible ? "" : "style='display:none'"}><td><img height='20' src='images/icone/fourmi.png'/></td><td>${numeral(this._armeeAv.getSommeUnite()).format()}</td><td><img height='20' src='images/icone/fourmi.png'/></td><td>${numeral(diffNbr).format("+0,0")} (${numeral(diffNbr / sommeAv).format("+0.00%")})</td><td><img height='20' src='images/icone/fourmi.png'/></td><td>${numeral(this._armeeAp.getSommeUnite()).format()}</td></tr>
            <tr ${bVisible ? "" : "style='display:none'"}><td>${IMG_VIE}</td><td>${numeral(this._armeeAv.getTotalVie(recherches[1])).format()}</td><td>${IMG_VIE}</td><td>${numeral(diffVie).format("+0,0")} (${numeral(diffVie / vieAv).format("+0.00%")})</td><td>${IMG_VIE}</td><td>${numeral(this._armeeAp.getTotalVie(recherches[1])).format()}</td></tr>
			<tr ${bVisible ? "" : "style='display:none'"}><td>${IMG_ATT}</td><td>${numeral(this._armeeAv.getTotalAtt(recherches[2])).format()}</td><td>${IMG_ATT}</td><td>${numeral(diffAtt).format("+0,0")} (${numeral(diffAtt / attAv).format("+0.00%")})</td><td>${IMG_ATT}</td><td>${numeral(this._armeeAp.getTotalAtt(recherches[2])).format()}</td></tr>
