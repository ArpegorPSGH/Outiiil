...
2.  Ajouter une condition pour vérifier si les paramètres `Commandes Outiiil` et `Membres Outiiil` sont configurés dans les paramètres de l'extension.
...
1.  **Test avec paramètres configurés et sujet membre existant :**
    *   S'assurer que les paramètres `Commandes Outiiil` et `Membres Outiiil` sont configurés.
...
3.  **Test avec paramètres non configurés :**
    *   S'assurer qu'au moins un des paramètres (`Commandes Outiiil` ou `Membres Outiiil`) n'est PAS configuré.
    *   Aller sur la page Commerce.
    *   Vérifier que le tableau des commandes n'est PAS affiché, indépendamment de l'existence d'un sujet membre.

## Avancement
La logique d'affichage du tableau des commandes sur la page Commerce (`js/page/Commerce.js`) a été modifiée. Le tableau ne s'affiche désormais que si les paramètres `Commandes Outiiil` et `Membres Outiiil` sont configurés ET que le joueur courant a un sujet dédié dans la section membres du forum configurée (vérifié via `js/page/Forum.js.verifierSujetMembre`). La fonctionnalité est terminée et fonctionnelle.
