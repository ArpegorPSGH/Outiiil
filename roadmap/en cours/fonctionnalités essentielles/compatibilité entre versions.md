...
- Page de fonctionnalité :
    ...
    - Chaque fonction membre de la classe de la page lancée dans executer, l'est via la fonction de vérification de versions dans une nouvelle classe dédiée, dans un nouveau fichier :
        ...
        - Si la version de l'extension est inférieure à la version minimale (calculée à l'aide des contenances) sur le forum pour au moins l'une des parties, déclencher une mise à jour de l'extension, et afficher un message de notification, puis recharger la page à la fin du message (s'il ne s'agit pas d'une tâche de fond, i. e. que l'objet qui a invoqué la vérification de version est une classe page)
    ...
    - Les versions minimales étant exprimées à différents niveaux, pour calculer la version minimale réelle d'une partie du forum, il faut prendre la plus restrictive (version plus récente) entre celle de cette partie et celles de toutes les parties qui la contiennent.
...
*   **4.2 Scénarios de Test de la Compatibilité des Versions pour les Pages de Fonctionnalité**
    *   **4.2.1 Scénario : Version de l'extension incompatible (mise à jour requise)**
        *   **Procédure :**
            1.  Modifier manuellement la version de l'extension dans `manifest.json` à une version *inférieure* (ex: "1.0.0").
            2.  Sur le forum de test, définir une version minimale pour une fonctionnalité spécifique (ex: `GestionPageAlliance`) à une version *supérieure* à celle de l'extension (ex: "1.1.0").
            3.  Recharger l'extension dans le navigateur.
            4.  Naviguer vers une page qui utilise cette fonctionnalité (ex: page Alliance).
            5.  **Vérification :**
                *   Observer qu'une notification claire de mise à jour est affichée à l'utilisateur.
                *   Vérifier que la page se recharge automatiquement après un court délai (pour forcer l'utilisateur à obtenir la nouvelle version).
                *   Vérifier que la fonctionnalité principale de la page (ex: affichage du tableau de l'alliance) n'est *pas* exécutée avant le rechargement.
    ...
    *   **4.2.3 Scénario : Échec de la mise à jour automatique**
        *   **Procédure :**
            1.  Mettre en place un environnement où la mise à jour automatique de l'extension échouerait (cela peut nécessiter de simuler un problème de réseau, un serveur de mise à jour inaccessible, ou de modifier temporairement le code de mise à jour pour qu'il échoue).
            2.  Définir une version minimale sur le forum supérieure à celle de l'extension.
            3.  Naviguer vers une page concernée.
            4.  **Vérification :**
